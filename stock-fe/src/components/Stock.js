...
const Stock = () => {
  ...
  useEffect(() => {
    ...
    // 在 component 初始化的時候只會跑一次
    // 所以通常會把去跟後端要資料的動作放在這裡
    // 這邊是跟下列網址要股票資料
  ...
  async function handleSubmit(e) {
    ...
    e.preventDefault(); // 不要有預設行為的設定
    let response = await axios.post('http://localhost:3001/api/stocks', {
      stockId,
      stockName,
    });
    
    setStocks([{id: stockId, name: stockName}, ...stocks]);
    ...
    <div>
      ...
      <form className="bg-purple-100 h-screen md:h-full md:my-20 md:mx-16 lg:mx-28 xl:mx-40 py-16 md:py-8 px-24 text-gray-800 md:shadow md:rounded flex flex-col md:justify-center">
        ...
        <div className="mb-4 text-2xl">
          ...
          <input className="w-full border-2 border-purple-200 rounded-md h-10 focus:outline-none focus:border-purple-400 px-2" type="text" id="stockId" name="stockId" value={stockId} 
          onChange={(e)=>{
